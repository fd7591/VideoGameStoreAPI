namespace API.Repositories.Repositories
{
    ...
    using VGS.Models.Enum;
    using VGS.Models.Request;
    using VGS.Models.Response;
    using VGS.Models.Shared;

    public class VideoGameRepositories : IVideoGameRepositories
    {
        private readonly IConfiguration _configuration;
        public VideoGameRepositories(IConfiguration configuration)
        {
        ...
        public async Task<VideoGameListResponse> GetAllVideoGames() {
            ...
            {
                response.OperationResult = new VGS.Models.Shared.OperationResult {
        ...
        public async Task<VideoGameResponse> PostVideoGame(VideoGameRequest request)
        {
            ...
            try {
                using (var conn = new SqlConnection(_configuration.GetConnectionString("storeConnectionString")))
                {
                    ...
                    command.Parameters.Add(new SqlParameter { ParameterName = "@IdConsole", Value = request.VideoGame.Console, SqlDbType = SqlDbType.Int });
                    command.Parameters.Add(new SqlParameter { ParameterName = "@IdGender", Value = request.VideoGame.Gender, SqlDbType = SqlDbType.Int });
